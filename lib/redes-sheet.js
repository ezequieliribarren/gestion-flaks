...
// Día de semana de una fecha, con lunes=1 ... domingo=7 (Date#getDay usa domingo=0).
function diaSemanaISO(anio, mes, dia) {
  const dow = new Date(anio, mes - 1, dia).getDay();
  return dow === 0 ? 7 : dow;
}

// Semanas "calendario" (lunes a domingo) de un mes, tal como se ven en un almanaque:
// la primera arranca el día 1 (sea cual sea su día de semana) y termina el domingo
// siguiente; la última termina el último día del mes aunque no caiga domingo. Así el
// número de semana coincide con la fila del calendario, no con bloques fijos de 7 días
// desde el 1° (que se desalinean con los lunes reales cuando el mes no arranca en lunes).
function semanasDelMes(anio, mes) {
  const totalDias = diasDelMes(anio, mes);
  const semanas = [];
  let desde = 1;
  let n = 1;
  while (desde <= totalDias) {
    const diasHastaDomingo = 8 - diaSemanaISO(anio, mes, desde);
    const hasta = Math.min(totalDias, desde + diasHastaDomingo - 1);
    semanas.push({ n, desde, hasta });
    desde = hasta + 1;
    n++;
  }
  return semanas;
...
// Número de semana (dentro de semanasDelMes) al que pertenece un día del mes.
function semanaDelDia(anio, mes, dia) {
  const s = semanasDelMes(anio, mes).find((x) => dia >= x.desde && dia <= x.hasta);
  return s ? s.n : null;
}

// Meta de posteos de una semana puntual: la propia (cargada para ese cliente/mes) si existe, si no la default.
function metaSemana(n, metasDelMes, defaultMeta) {
  const propia = metasDelMes && metasDelMes[n];
  if (propia != null && propia !== '' && !isNaN(propia) && Number(propia) > 0) return Number(propia);
  return defaultMeta;
}

// Período ('YYYY-MM') y cantidad de semanas del mes actual (4 o 5, a veces 6), para saber
// cuántos casilleros de meta mostrar antes incluso de leer el Sheet.
function periodoActual() {
  ...
  return {
    ...
    semanaActual: semanaDelDia(anio, mes, dia),
  };
}
...
// Calcula el resumen de avance de un cliente a partir de su Sheet, para el mes actual
// por defecto (o para anioObjetivo/mesObjetivo si se pasan, ej. para revisar un mes que
// ya cerró desde una tarea automática).
// metasDelMes: { [numSemana]: meta } ya resuelto para ese período (redes_metas_semana).
// Devuelve { ok:true, posteos:{...}, historias:{...} } o { ok:false, error }.
async function resumenCliente(cliente, metasDelMes, anioObjetivo, mesObjetivo) {
  ...
  const anio = anioObjetivo || actual.anio;
  const mesActual = mesObjetivo || actual.mes;
  const esMesActual = anio === actual.anio && mesActual === actual.mes;

  const semanas = semanasDelMes(anio, mesActual).map((s) => ({
    ...s, cantidad: 0, meta: metaSemana(s.n, metasDelMes, defaultMetaPosteosSem),
  }));
  ...
  for (let r = cols.headerRow + 1; r < filas.length; r++) {
    ...
    if (fp && fp.mes === mesActual) {
      posteosMes++;
      const s = semanaDelDia(anio, mesActual, fp.dia);
  ...
  return {
    ...
    posteos: {
      ...
      semana_actual: esMesActual ? actual.semanaActual : null,
      semanas,
