...
// Período ('YYYY-MM') y cantidad de semanas del mes actual (4 o 5), para saber
...
function periodoActual() {
  ...
  return {
    ...
    mes,
    dia,
    totalSemanas: semanaDe(totalDias),
    semanaActual: semanaDe(dia),
  };
}
...
// metasDelMes: { [numSemana]: meta } ya resuelto para el período actual (redes_metas_semana).
// Devuelve { ok:true, posteos:{...}, historias:{...} } o { ok:false, error }.
async function resumenCliente(cliente, metasDelMes) {
  ...
  const { anio, mes: mesActual, totalSemanas, semanaActual } = periodoActual();
  ...
  const semanas = Array.from({ length: totalSemanas }, (_, i) => {
    ...
    return { n, desde, hasta, cantidad: 0, meta: metaSemana(n, metasDelMes, defaultMetaPosteosSem) };
  });
...
// Días (1=lunes..5=viernes) de la semana actual en que este cliente ya tiene
// al menos una publicación cargada en el Sheet. Para la tarjeta de Inicio.
async function diasPublicadosSemanaActual(cliente) {
  if (!cliente || !cliente.redes_sheet_url) return new Set();
  let filas;
  try {
    filas = await descargarFilas(cliente.redes_sheet_url);
  } catch (e) {
  ...
  const { anio, mes, semanaActual } = periodoActual();
  const dias = new Set();
  for (let r = cols.headerRow + 1; r < filas.length; r++) {
    const fp = parseDiaMes((filas[r] || [])[cols.pubFechaCol]);
    if (fp && fp.mes === mes && semanaDe(fp.dia) === semanaActual) {
      const diaSemana = new Date(anio, mes - 1, fp.dia).getDay(); // 0=domingo..6=sábado
      const isoSemana = diaSemana === 0 ? 7 : diaSemana; // 1=lunes..7=domingo
      if (isoSemana <= 5) dias.add(isoSemana);
    }
  }
  return dias;
}
