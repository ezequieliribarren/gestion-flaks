...
// Notifica a todos los usuarios admin (rol distinto de 'contenido').
function notificarAdmins(tipo, refId, texto, url, deQuien) {
  const admins = db.prepare("SELECT id FROM users WHERE rol <> 'contenido'").all();
  for (const a of admins) {
    insNotif.run({ user_id: a.id, tipo, ref_id: refId, texto, url: url || null, de_quien: deQuien || null });
  }
}

module.exports = {
  usuarios, asignadosDe, asignadosIds, setAsignados,
  notificarParticipantes, notificarAdmins, contarNoLeidas,
};
