...
function facturacionDelMes(anio, mes, opts = {}) {
  ...
  const unicos = db.prepare(`
    ...
    WHERE substr(tu.fecha, 1, 7) = ?
      AND (tu.estado IN ('realizado', 'adeuda') OR ? = 1)
  ...
  for (const u of unicos) {
    ...
    if (u.trabajo_estado === 'adeuda') f.pendiente = true;
    if (u.trabajo_estado === 'realizado') f.cobrado = true;
    filas.push(f);
  }
  ...
  const totales = filas.reduce(
    (acc, f) => {
      ...
      if (f.pendiente) acc.pendiente += f.monto;
      if (f.cobrado) acc.cobrado += f.monto;
      return acc;
    },
