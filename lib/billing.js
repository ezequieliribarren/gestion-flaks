...
// Monto de un recurrente vigente en un período puntual ('YYYY-MM'), según su
// historial de precios (recurrente_montos). Si todavía no tiene historial cargado
// para ese momento (ej. nunca se editó desde que existe esta función), usa el monto
// actual del recurrente como mejor aproximación disponible.
function montoVigente(recurrenteId, periodo, montoActual) {
  const fila = db.prepare(
    'SELECT monto FROM recurrente_montos WHERE recurrente_id = ? AND vigente_desde <= ? ORDER BY vigente_desde DESC LIMIT 1'
  ).get(recurrenteId, periodo);
  return fila ? fila.monto : montoActual;
}

// Registra un cambio de precio de un recurrente sin alterar los meses ya facturados:
// la primera vez que se edita, "congela" el monto anterior desde siempre (para que
// los meses pasados sigan viéndose como se facturaron), y el nuevo monto rige desde
// el mes en curso en adelante.
function registrarCambioMonto(recurrenteId, montoAnterior, montoNuevo, periodoDesde, usuario) {
  if (montoAnterior === montoNuevo) return;
  const tieneHistorial = db.prepare('SELECT 1 FROM recurrente_montos WHERE recurrente_id = ? LIMIT 1').get(recurrenteId);
  if (!tieneHistorial) {
    db.prepare('INSERT INTO recurrente_montos (recurrente_id, monto, vigente_desde, creado_por) VALUES (?, ?, ?, ?)')
      .run(recurrenteId, montoAnterior, '0000-01', usuario);
  }
  db.prepare(`
    INSERT INTO recurrente_montos (recurrente_id, monto, vigente_desde, creado_por) VALUES (?, ?, ?, ?)
    ON CONFLICT(recurrente_id, vigente_desde) DO UPDATE SET monto = excluded.monto, creado_por = excluded.creado_por
  `).run(recurrenteId, montoNuevo, periodoDesde, usuario);
}
...
function facturacionDelMes(anio, mes, opts = {}) {
  ...
  const filas = [];
  for (const r0 of recurrentes) {
    const r = { ...r0, monto: montoVigente(r0.id, prefijo, r0.monto) };
